      ...
      try {
        ...
        if (userData) {
          ...
          if (parsedData) {
            ...
            setProfilePic(parsedData.picture || 'https://via.placeholder.com/100'); // Load profile picture or placeholder
          }
        }
      } catch (error) {
  ...
  return (
    <DrawerContentScrollView {...props}>
      <View style={styles.drawerHeader}>
        <Image 
          source={{ uri: profilePic }} // Load profile picture from AsyncStorage
          style={{ width: 100, height: 100, borderRadius: 50 }}
        />
        {/* Ensure that both username and email are wrapped in a <Text> component */}
        <Text style={styles.drawerHeaderText}>{userName ? userName : 'No Name'}</Text> 
        <Text style={styles.drawerEmailText}>{userEmail ? userEmail : 'No Email'}</Text>
      </View>
